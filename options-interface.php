	...
	foreach ( $options as $value ) {
		...
		case "images":
			...
			foreach ( $value['options'] as $key => $option ) {
				...
				$output .= '<img id="img_'. esc_attr($value['id'] .'_'. $key) .'" src="' . esc_url( $option ) . '" alt="' . $option .'" class="of-radio-img-img' . $selected .'" onclick="document.getElementById(\''. esc_attr($value['id'] .'_'. $key) .'\').checked=true;" />';
			}
			break;
		...
		case "docs":
			...
			if (is_child_theme()) {
					$output .= '<a style="position:absolute;top:20px;right:20px;" class="button-primary" href="'.$_SERVER['PHP_SELF'].'?page=options-framework&update_check">"'.__('Check for Updates.', 'jewelux').'"</a>';
			}

			$output .= Markdown($docfile);
			}
		...
		case "multicheck":
			...
			break;
                ...
                case "social":
                    $single_val = '';
                    $placeholder = '';
                    $output .= '<div id="' . $value['id'] . '" class="sort_social">';
                    $social_icon_order = get_option($value['id'] . '_order');
                    $i = 0;
                    if ($social_icon_order == '') {
                        foreach ($value['options'] as $key => $option) {
                            $social_icon_order[$key] = $i;
                            $i++;
                        }
                    }
                    if ($social_icon_order != '') {
                        foreach ($social_icon_order as $icon => $order) {
                            if (isset($value['placeholder'])) {
                                $placeholder = $value['placeholder'][$icon];
                            }
                            $field_id = $option_name . '-' . $value['id'] . '-' . $value['options'][$icon];
                            $field_name = $option_name . '[' . $value['id'] . '][' . $value['options'][$icon] . ']';
                            if (isset($val[$icon]))
                                $single_val = $val[$icon];
                            $output .= '<div id="si__' . $icon . '" class="' . $value['class'] . '">';
                            $output .= '<h4 class="heading">' . $value['labels'][$icon] . '</h4>';
                            $output .= '<div class="social-icons">';
                            $output .= '<input placeholder="' . $placeholder . '" id="' . esc_attr($field_id) . '" class="of-input" type="text" style="width: 100%" value="' . $single_val . '" name="' . esc_attr($field_name) . '">';
                            $output .= '</div>';
                            $output .= '</div>';
                        }
                    }
		...
		case "info":
			...
			if ( isset($value['name']) ) {
				$output .= '<h3 class="heading" style="font-size: 18px; margin-bottom: 0px;">' . esc_html( $value['name'] ) . '</h3>' . "\n";
			}
			if ( isset($value['desc']) ) {
